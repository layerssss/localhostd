...
class Server {
  ...
  constructor(options) {
    ...
    this._proxyServer.on("connect", (request, socket, head) =>
      this._handleConnect(request, socket, head)
    );
    this._proxyServer.on("upgrade", (request, socket, head) =>
    ...
    this._bnbServer.on("request", (request, response) =>
  ...
  listen(options) {
    return Promise.resolve()
      ...
      .then(
        () =>
          new Promise(resolve =>
            this._bnbServer.listen(this._bnbServerPort, "localhost", resolve)
          )
      )
      .then(() =>
        Portfinder.getPortPromise({
      ...
      .then(
        () =>
          new Promise(resolve =>
            this._sslServer.listen(this._sslServerPort, "localhost", resolve)
          )
      )
      .then(
        () =>
          new Promise(resolve =>
            this._proxyServer.listen(options.port, options.bind, resolve)
          )
      )
      ...
      .then(address => {
        console.log(
          "listening to http://" + address.address + ":" + address.port
        );
        this._bnb.bind = address.address.replace(
          /0\.0\.0\.0|127\.0\.0\.1/,
          "localhost"
        );
        this._bnb.port = address.port;
        this._bnb._broadcastStates();
      });
  }
  ...
  _initializePki() {
    ...
    return Promise.resolve()
      .then(() => {
        ...
        return Utility.createKeys().then(keys => {
          serverKeys = keys;
  ...
  _handleSNICallback(domain, cb) {
    ...
    Promise.resolve()
      .then(
        () =>
          !domain.match(/\.dev$/) &&
          Promise.reject(new Error(`${domain} is not configured.`))
      )
      .then(() => this._initializePki())
      .then(pkiInfo => {
      ...
      .then(() => {
        ...
        return Utility.createDomainCertificate(
          serverKeys,
          caCertificate,
          domain,
          expire
  ...
  _transformRequestHeaders(request, ssl) {
    ...
    if (request.headers["connection"] != "Upgrade")
      _.unset(request.headers, "connection");

    for (var key in request.headers) {
      if (!request.headers[key]) delete request.headers[key];
    }
  }

  _transformResponseHeaders(response, ssl) {
    if (response.headers["connection"] != "Upgrade")
      _.unset(response.headers, "connection");
    response.headers["X-Bnb-Version"] = require("../package.json").version;

    for (var key in response.headers) {
      if (!response.headers[key]) delete response.headers[key];
    }
  }
    ...
    Promise.resolve()
      .then(() => {
        ...
        if (_.endsWith(hostname, ".dev")) {
          ...
          return this._bnb.getApplicationPort(hostname).then(applactionPort => {
            if (!applactionPort)
              return Promise.reject(new Error(`${hostname} not configured.`));
  ...
  _handleUpgrade(request, socket, head, ssl) {
    ...
    Promise.resolve()
      .then(() => {
        ...
        return this._bnb.getApplicationPort(hostname).then(port => {
          if (!port)
            return Promise.reject(new Error(`${hostname} not configured.`));
      ...
      .then(() => {
        ...
        return new Promise((resolve, reject) => {
          ...
          clientRequest.on("upgrade", (clientResponse, clientSocket) =>
            resolve({
              clientResponse,
              clientSocket
            })
      ...
      .then(({ clientResponse, clientSocket }) => {
  ...
  _handleRequest(request, response, ssl) {
    ...
    if (!url.hostname) {
      [clientHostname, clientPort] = (request.headers.host || "").split(":");
    ...
    return Promise.resolve()
      .then(() => {
        ...
        if (_.endsWith(clientHostname, ".dev")) {
          return this._bnb.getApplicationPort(clientHostname).then(port => {
            if (!port)
              return Promise.reject(
                new Error(`${clientHostname} not configured.`)
              );
