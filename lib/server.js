...
class Server {
  ...
  listen(options) {
    return Promise.resolve()
      ...
      .then(address => {
        ...
        this._bnb.bind = address.address.replace(/0\.0\.0\.0|127\.0\.0\.1/, 'localhost');
  ...
  _handleSNICallback(domain, cb) {
    ...
    var domainSsl;
    Promise.resolve()
      .then(() => !domain.match(/\.dev$/) && Promise.reject(new Error(`${domain} is not configured.`)))
      .then(() => this._initializePki())
      .then(pkiInfo => {
  ...
  _transformRequestHeaders(request, ssl) {
    ...
    if (request.headers['host']) _.defaults({
