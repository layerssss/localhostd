...
class Server {
  ...
  constructor(options) {
    ...
    this._motelServer = Http.createServer();
    this._motelServer.on('request', (request, response) => express(request, response));
    this._motelServer.on('upgrade', (request, socket, head) =>
  ...
  handleWebsocket(websocket) {
    this._motel.handleWebsocket(websocket);
  }
  ...
  listen(options) {
    return Promise.resolve()
      ...
      .then((port) => this._motelServerPort = port)
      .then(() => new Promise(resolve =>
        this._motelServer.listen(
          this._motelServerPort,
          'localhost',
          resolve
        )
      ))
  ...
  close() {
    return Promise.resolve()
      .then(() => this._motel.ensureDown())
      .then(() => {
        this._motelServer.close();
        this._motelServer.unref();
  ...
  _handleSNICallback(domain, cb) {
    ...
    Promise.resolve()
      .then(() => { // ensure domain valid
        if (domain == 'motel.dev') return;
        return this._motel.getApplicationPort(domain)
          .then(port => {
  ...
  _transformResponseHeaders(response, ssl) {
    ...
    response.headers['X-Motel-Version'] = require('../package.json').version;
  }
    ...
    Promise.resolve()
      .then(() => {
        ...
        if (_.endsWith(hostname, '.dev')) {
          ...
          if (hostname == 'motel.dev') {
            clientHostname = 'localhost';
            clientPort = this._motelServerPort;
            return;
          }

          return this._motel.getApplicationPort(hostname)
            .then(applactionPort => {
  ...
  _handleUpgrade(request, socket, head, ssl) {
    ...
    Promise.resolve()
      .then(() => {
        ...
        if (hostname == 'motel.dev') return clientPort = this._motelServerPort;
    ...
    return Promise.resolve()
      .then(() => {
        ...
        if (_.endsWith(clientHostname, '.dev')) {
          return this._motel.getApplicationPort(clientHostname)
            .then(port => {
