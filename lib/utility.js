...
module.exports = class Utility {
  ...
  static which(path, options) {
    return new Promise((resolve, reject) => {
      Which(path, options, (error, result) => {
        if (error) return reject(error);
        resolve(result);
      });
    });
  ...
  static whilePromise(test) {
    return Promise.resolve()
      ...
      .then(
        result =>
          result && Utility.delay(100).then(() => Utility.whilePromise(test))
      );
  }
  ...
  static lockPromise(key, callback) {
    ...
    return Promise.resolve()
      ...
      .then(
        () => (callbackError ? Promise.reject(callbackError) : callbackResult)
      );
  }
  ...
  static createCaCertificate(recipient, keys, expire) {
    return Promise.resolve().then(() => {
      ...
      var attrs = [
        {
          name: "commonName",
          value: `Bnb Development Certificate Authority for ${recipient}`
        }
      ];
