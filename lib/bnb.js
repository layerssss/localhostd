...
class Bnb {
  ...
  _ensureApplicationUp(application) {
    if (application.timeout) {
      if (this._applicationTimers[application.name]) clearTimeout(this._applicationTimers[application.name]);
    ...
    return this._lockApplication(application, () => {
      ...
      return Promise.resolve()
        ...
        .then(stat => !stat.isDirectory() && Promise.reject(new Error(`${application.dir} is not a directory`)))
        .then(()=> !executable.match(/\//) && Utility.which(executable, {
            path: envPath,
            pathExt: env['PathExt']
          })
          .catch(error => Promise.reject(new Error(`Cannot find executable ${executable} in PATH (${envPath})`)))
        )
        .then(() => {
          ...
          terminal.pty.on('exit', code => {
            ...
            this._sendTerminalOutput(application, `${executable} exited with ${code}.\r\n`);
            this._broadcastStates();
          });
