...
class Bnb {
  constructor({ state, env, onStateChange }) {
    this._env = env;
    this._state = state;
  ...
  _lockApplication(application, callback) {
    ...
    return Promise.resolve()
      .then(() =>
        Utility.whilePromise(() => this._applicationLocks[application.name])
      )
  ...
  getApplicationPort(hostname) {
    var application = _.find(
      this._state.applications,
      a => hostname == `${a.name}.dev` || _.endsWith(hostname, `.${a.name}.dev`)
    );
    return Promise.resolve().then(
      () =>
        application &&
        this._ensureApplicationUp(application).then(() => application.port)
    );
  }
  ...
  _doActionStartApplication(parameters) {
    var application = _.find(
      this._state.applications,
      a => a.name == parameters.applicationName
    );
  ...
  _doActionRestartApplication(parameters) {
    ...
    return Promise.resolve()
      ...
      .then(() => this._ensureApplicationUp(application))
      .then(error =>
        this._broadcastMessage("success", "Application restarted.")
      )
      .catch(error => this._broadcastMessage("danger", error.message));
  }
  ...
  _doActionCreateApplication(parameters, session) {
    var application = _.find(
      this._state.applications,
      a => a.name == parameters.application.name
    );

    return Promise.resolve()
      .then(() => application && this._ensureApplicationDown(application))
      .then(
        () => !application && this._state.applications.push((application = {}))
      )
    ...
    return this._lockApplication(application, () => {
      ...
      return Promise.resolve()
        ...
        .then(
          stat =>
            !stat.isDirectory() &&
            Promise.reject(new Error(`${application.dir} is not a directory`))
        )
        .then(
          () =>
            !executable.match(/\//) &&
            Utility.which(executable, {
              path: envPath,
              pathExt: env["PathExt"]
            }).catch(error =>
              Promise.reject(
        ...
        .then(() => {
          ...
          if (application.out) {
            ...
            Promise.resolve()
              .then(() => Utility.mkdirp(Path.dirname(outPath)))
              .then(() => Utility.openFile(outPath, "a"))
          ...
          terminal.pty.on("data", data => {
            ...
            this._sendTerminalOutput(application, data);
          });
        ...
        .then(() =>
          Utility.whilePromise(() => {
            var terminal = this._terminals[application.name];
            if (!terminal) return;
            if (Date.now() - startTime > 5000) return;

            return Utility.checkPortListening(application.port).then(
              listening => !listening
            );
          })
        )
        ...
        .catch(error => {
          this._sendTerminalOutput(application, `${error.message}\r\n`);

          return Promise.reject(new Error("Failed to launch to application."));
        });
    });
  }
  ...
  _ensureApplicationDown(application) {
    ...
    return this._lockApplication(application, () => {
      ...
      return Utility.whilePromise(() => this._terminals[application.name]).then(
  ...
  ensureDown() {
    return Promise.all(
      this._state.applications.map(application =>
        this._ensureApplicationDown(application)
      )
    );
  }
