...
class LocalhostD {
  ...
  _findApplication(hostname) {
    return this._state.applications.find(
      a =>
  ...
  async _ensureApplicationUp(application) {
    await this._lockApplication(application, async () => {
      ...
      const dirEnv = await getShellEnv({ cwd: application.dir });
        ...
        JSON.stringify({
          state: {
            ...
            applications: this._state.applications.map(application => ({
              ...application,
              origin: `http://${this._getApplicationHostname(application)}`,
