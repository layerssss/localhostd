...
import LocalhostD from "./LocalhostD.js";
import ProxyCore from "./ProxyCore.js";
import createKeys from "./createKeys.js";
import createCaCertificate from "./createCaCertificate.js";
import createDomainCertificate from "./createDomainCertificate.js";
...
class Server {
  ...
  constructor(options) {
    ...
    this._proxyServer.on("request", (request, response) =>
      this._handleProxyRequest(request, response)
    );
    ...
    this._uiServerExpress.use((request, response, _next) => {
      response.writeHead(302, {
        Location: `/?redirect=${request.url}`,
      });
      response.end();
    });
  ...
  _handleProxyConnect(request, socket, head) {
    Promise.resolve()
      .then(async () => {
        ...
        if (application) {
          this._localhostd.pingApplication(hostname);
  ...
  _handleProxyRequest(request, response) {
    Promise.resolve()
      .then(async () => {
  ...
  _handleUpgrade(request, socket, head, ssl) {
    Promise.resolve()
      .then(async () => {
        ...
        this._localhostd.pingApplication(host);
        if (application.timeout) {
          const pingTimer = setInterval(() => {
            this._localhostd.pingApplication(host);
          }, application.timeout * 1000 - 10);
        ...
        this._debug(
          "proxyUpgrade",
          `${request.method} localhost:${application.port}${request.url}`
        );
